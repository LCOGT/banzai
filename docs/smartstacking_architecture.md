...
Next, `banzai-download-worker` polls the replicated rows. For the configured site and instrument types, it keeps the two newest non-bad masters with archive frame IDs for each instrument and calibration-specific configuration. It downloads missing FITS files through the Archive API into `HOST_CALS_DIR`, records or reconciles their local directories in `calimages.filepath`, and prunes previously tracked files that leave the retained set. Pruning waits until all required downloads are complete, including those on cooldown; one unavailable replacement can therefore retain extra old files across the cache.

After normal cache work, the worker also examines the newest two good masters per configuration without requiring archive IDs. It resolves missing IDs using BANZAI's existing filename/date search and saves them in the local database only if the stored ID is still NULL. Recovered IDs are eligible for download on the next pass. It attempts at most ten lookups per pass; missing matches and lookup errors share the existing two-hour, in-memory cooldown with download failures. Unresolved candidates do not displace the archived calibrations selected for retention. Replication can overwrite local repairs, and restarting the worker clears the cooldown.

By default, stackframe reduction selects an applicable master from the local database and reads its FITS file from the cache. If the cached path is unavailable, frame opening can fall back to the archive without filling the cache. Reuse waits for the download worker, whose default pause is ten seconds between completed passes. Older calibrations outside the newest-two candidates are not repaired by this process. This cache lifecycle is independent of `STACK_RETENTION_DAYS` and Smartstack database cleanup.
