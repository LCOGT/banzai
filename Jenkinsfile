...
pipeline {
	...
	stages {
		...
		stage('Test-Master-Bias-Creation') {
			...
			steps {
				script {
					sh('kubectl --kubeconfig=${KUBERNETES_CREDS} -n dev exec -p banzai-e2e-test -c BANZAITestRunner -- banzai_run_end_to_end_tests --marker=master_bias --junit-file=/archive/engineering/pytest-master-bias.xml --code-path=/lco/banzai')
				}
			}
			post {
				always {
		...
		stage('Test-Master-Dark-Creation') {
			...
			steps {
				script {
					sh('kubectl --kubeconfig=${KUBERNETES_CREDS} -n dev exec -p banzai-e2e-test -c BANZAITestRunner -- banzai_run_end_to_end_tests --marker=master_dark --junit-file=/archive/engineering/pytest-master-dark.xml --code-path=/lco/banzai')
				}
			}
			post {
				always {
		...
		stage('Test-Master-Flat-Creation') {
			...
			steps {
				script {
					sh('kubectl --kubeconfig=${KUBERNETES_CREDS} -n dev exec -p banzai-e2e-test -c BANZAITestRunner -- banzai_run_end_to_end_tests --marker=master_flat --junit-file=/archive/engineering/pytest-master-flat.xml --code-path=/lco/banzai')
				}
			}
			post {
				always {
		...
		stage('Test-Science-File-Creation') {
			...
			steps {
				script {
					sh('kubectl --kubeconfig=${KUBERNETES_CREDS} -n dev exec -p banzai-e2e-test -c BANZAITestRunner -- banzai_run_end_to_end_tests --marker=science_files --junit-file=/archive/engineering/pytest-science-files.xml --code-path=/lco/banzai')
				}
			}
			post {
				always {
