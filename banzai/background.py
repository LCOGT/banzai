...
class SkyBackgroundTest(Stage):
    ...
    def __init__(self, pipeline_context):
        super(SkyBackgroundTest, self).__init__(pipeline_context)
...
def box_statistics(image):
    ...
    xposition = np.arange(float(img_xmin)+dxpix, float(img_xmax), dxpix)
    ...
    sky_mean = np.array(sky_mean)
    sky_stddev = np.array(sky_stddev)
