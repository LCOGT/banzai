...
def trace_celery_task() -> Callable:
    """
    Decorator specifically for Celery tasks that adds Celery specific attributes

    @trace_celery_task()
    def my_celery_task(self, arg1, arg2):
        pass
    """
    def decorator(func: Callable) -> Callable:
        @functools.wraps(func)
        def wrapper(self, *args, **kwargs):
            ...
            with tracer.start_as_current_span(name) as span:
                try:
                    # Add Celery-specific attributes
                    span.set_attribute("celery.task.name", func.__name__)
                    span.set_attribute("celery.task.module", func.__module__)
                    ...
                    # Add argument information
                    if args:
                        span.set_attribute("celery.task.args_count", len(args))
                        # Add specific argument info for common patterns
                        if len(args) > 0:
                            first_arg = args[0]
                            if isinstance(first_arg, dict) and 'filename' in first_arg:
                                span.set_attribute("celery.task.filename", first_arg['filename'])
                            elif isinstance(first_arg, str):
                                span.set_attribute("celery.task.first_arg", first_arg[:100])

                    if kwargs:
                        span.set_attribute("celery.task.kwargs_count", len(kwargs))
                    ...
                    span.set_attribute("error.message", str(e)[:200])  # Limit length
