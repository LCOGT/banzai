...
#TODO: Figure out the need for both of these?
extension_headers = [{'DATASEC': '[1:512,1:512]', 'DETSEC': '[1025:2048,3072:2049]', 'CCDSUM': '2 2'},
...
binned_windowed_extension_headers = [{'DATASEC': '[1:512,1:512]', 'DETSEC': '[1025:2048,3072:2049]', 'CCDSUM': '2 2'},
                                     {'DATASEC': '[1:512,1:512]', 'DETSEC': '[1025:2048,1025:2048]', 'CCDSUM': '2 2'},
                                     {'DATASEC': '[1:512,1:512]', 'DETSEC': '[3072:2049,1025:2048]', 'CCDSUM': '2 2'},
                                     {'DATASEC': '[1:512,1:512]', 'DETSEC': '[3072:2049,3072:2049]', 'CCDSUM': '2 2'}]
...
def test_mosaic_size():
    ...
    assert image.get_mosaic_size() == (1024, 1024)


def test_mosaic_size_for_binned_windowed_mode():
    data = [FakeCCDData(meta=extension_header) for extension_header in binned_windowed_extension_headers]
    image = FakeLCOObservationFrame(hdu_list=data)
    assert image.get_mosaic_size() == (1024, 1024)
...
def test_mosaic_maker_for_binned_windowed_mode():
    ...
    data = [FakeCCDData(meta=extension_header, 
                        data=np.random.uniform(0, 1, size=(512,512)),
                        mask=np.random.choice([0, 1], size=(512,512))) for extension_header in binned_windowed_extension_headers]
