...
def test_adds_good_bpm(mock_bpm_name, mock_bpm, mock_update_frameid, set_random_seed):
...
@mock.patch('banzai.lco.LCOFrameFactory.open')
@mock.patch('banzai.calibrations.CalibrationUser.get_calibration_file_info', return_value={'filename': 'test.fits'})
def test_adds_good_bpm_3d(mock_bpm_name, mock_bpm, mock_update_frameid, set_random_seed):
...
@mock.patch('banzai.lco.LCOFrameFactory.open')
@mock.patch('banzai.calibrations.CalibrationUser.get_calibration_file_info', return_value={'filename': 'test.fits'})
def test_removes_image_if_wrong_shape(mock_get_bpm_filename, mock_bpm, mock_update_frameid, set_random_seed):
    image = FakeLCOObservationFrame(hdu_list=[FakeCCDData(memmap=False)])
...
@mock.patch('banzai.lco.LCOFrameFactory.open')
@mock.patch('banzai.calibrations.CalibrationUser.get_calibration_file_info', return_value={'filename': 'test.fits'})
def test_removes_image_wrong_shape_3d(mock_get_bpm_filename, mock_bpm, mock_update_frameid, set_random_seed):
    image = FakeLCOObservationFrame(hdu_list=[FakeCCDData(memmap=False)])
